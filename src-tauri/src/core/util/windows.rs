...
use windows::Win32::System::ProcessStatus::{K32GetModuleFileNameExW, OpenProcess};
...
pub fn get_local_installed_apps(app_handle: &tauri::AppHandle) -> Vec<String> {
    ...
    unsafe {
        ...
        let result = RegOpenKeyExW(HKEY_LOCAL_MACHINE, uninstall_key, 0, KEY_READ, &mut hkey);
        ...
        if result.is_ok() {
            ...
            .is_ok()
            {
                ...
                let full_key = format!("{}\\{}", uninstall_key, app_key);

                // 打开子键
                let mut app_hkey = HKEY_LOCAL_MACHINE;
                if RegOpenKeyExW(HKEY_LOCAL_MACHINE, &full_key, 0, KEY_READ, &mut app_hkey).is_ok()
                {
                    // 读取显示名称
